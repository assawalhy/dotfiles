...
- [ ] 3. Rewrite lua/plugins/lsp.lua on the modern lspconfig 1.x API
  What to do / Must NOT do:
...
- [ ] 4. Create lua/plugins/conform.lua and lua/plugins/trouble.lua
  What to do / Must NOT do:
...
- [ ] 5. Create after/plugin/lsp.lua (LspAttach native keymaps) and fix ufo K fallback
  What to do / Must NOT do:
...
- [ ] 6. Migrate completion to blink.cmp
  What to do / Must NOT do:
...
- [ ] 7. Add maven and gradle to setup/packages.list
  What to do / Must NOT do:
...
- [ ] 8. Add java debug/test adapters to dap.lua
  What to do / Must NOT do:
...
- [ ] 9. Add nvim-jdtls plugin and telescope jdt:// URI patch
  What to do / Must NOT do:
...
- [ ] 10. Create after/ftplugin/java.lua with full jdtls config
  What to do / Must NOT do:
...
- [ ] 11. Java QA: sample Maven project + headless attach verification
  What to do / Must NOT do:
...
- [ ] 12. Install stylua and format all lua files
  What to do / Must NOT do:
...
- [ ] 13. Lazy clean orphans + refresh lazy-lock.json
  What to do / Must NOT do:
...
- [ ] 14. Link new files into ~/.config/nvim
  What to do / Must NOT do:
...
- [ ] 15. README: document nvim config structure and Java setup
  What to do / Must NOT do:
...
- [ ] 16. Headless smoke test suite for the nvim config
  What to do / Must NOT do:
...
- [ ] F1. Plan compliance audit — every todo's acceptance criteria re-run against the final tree; evidence files exist for all 16 todos; no scope creep (diff shows only the files named in the plan).
- [ ] F2. Code quality review — read every changed/created lua file: no dead code, no Lspsaga/neodev/nvim-cmp remnants, no stubs, stylua-clean, desc on every keymap, no commented-out blocks left behind.
- [ ] F3. Real manual QA — boot nvim interactively (tmux), open a java file from the sample project, verify: jdtls attaches (`:LspInfo`), completion pops (blink), `;rn`/`;ac`/`gd` work, `:Trouble` opens, `:Format` works, `:Mason` shows jdtls/java-debug-adapter/java-test installed.
- [ ] F4. Scope fidelity — user's feature set intact (DAP, ufo, textobjects, competitest, neotree, vgit, codeium, clipboard clip flow); keybinding semantics unchanged; dirty-worktree user changes not reverted; nothing outside common/.config/nvim + setup/packages.list + README.md + tests/nvim-smoke.bash modified.
